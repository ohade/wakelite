...
- Creation applies the AMQ default only to new resumable cmux callbacks. Explicit `"amq": false` is preserved, updates do not silently opt in an existing timer, and already-stored legacy timers remain unchanged.
