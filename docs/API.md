...
- The wake check and `amq send` each have a 5-second process timeout, so a degraded AMQ path can add up to roughly 10 seconds before direct cmux fallback. AMQ-to-fallback delivery is at-least-once: if `amq send` exceeds its timeout after storing the body, WakeLite falls back and the stored message may still be drained later, exposing the same `run_id` twice.
